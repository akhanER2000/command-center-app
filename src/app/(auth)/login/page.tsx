...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background p-4 relative overflow-hidden">
      {/* Absolute background glows based on image inspiration */}
      ...
      <div className="w-full max-w-md space-y-8 glass rounded-3xl p-10 relative z-10 shadow-2xl">
        <div className="text-center space-y-2">
          <h1 className="text-4xl font-black tracking-tighter gradient-text">Command</h1>
          <p className="text-muted-foreground font-medium">Inicia sesión en tu centro de control</p>
        </div>
        ...
        <div className="relative">
          ...
          <div className="relative flex justify-center text-[10px] font-bold uppercase tracking-widest">
            <span className="bg-[#0f0f1b] px-4 text-muted-foreground/60 rounded-full">O continúa con</span>
          </div>
        </div>

        <button
          onClick={handleGithubLogin}
          className="w-full btn btn-outline py-4 flex items-center justify-center gap-3 border-2 hover:border-primary/50 transition-all"
        >
          ...
          <span className="font-bold">GitHub Access</span>
        </button>

        <p className="text-center text-sm font-medium text-muted-foreground">
          ¿No tienes cuenta?{' '}
          <Link href="/register" className="text-primary hover:text-accent transition-colors font-bold underline-offset-4 hover:underline">
            Crea una aquí
          </Link>
        </p>
      </div>
